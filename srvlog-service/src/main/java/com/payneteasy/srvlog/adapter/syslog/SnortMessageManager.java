...
public class SnortMessageManager {
    ...
    public void processOssecSnortMessage(String rawMessage, LogData logData) {
        ...
        unprocessedSnortLogs.stream().forEach((unprocessedSnortLog) -> {
            saveSnortLog(unprocessedSnortLog.getMessage(), ossecSnortMessage.getHash());
        });

        logData.setHash(ossecSnortMessage.getHash());

        if (!unprocessedSnortLogs.isEmpty()) {
            logData.hasSnortLogs(true);
        }
    ...
    public void processRawSnortMessage(String rawMessage) {
        RawSnortMessage rawSnortMessage = createRawSnortMessage(rawMessage);

        List<OssecLogData> ossecLogDatas = logCollector.getOssecLogs(rawSnortMessage);

        if (ossecLogDatas.isEmpty()) {
            logCollector.saveUnprocessedSnortLog(rawSnortMessage);
            return;
        }
        
        ossecLogDatas.stream().forEach((ossecLogData) -> {
            saveSnortLog(rawSnortMessage.getMessage(), ossecLogData.getHash());
            logCollector.changeHasSnortLogs(ossecLogData.getLogId(), true);
        });
    }
    
    /**
     * Saves message that generated by snort and received from barnyard2, 
     * that associated with message that generated by ossec and contains message from snort.
     * 
     * @param       rawMessage      Raw message that generated by snort and received from barnyard2.
     * @param       hash            Hashed for message that generated by ossec and contains message from snort.
     */
    private void saveSnortLog(String rawMessage, String hash) {
        SnortLogData snortLog = createSnortMessage(rawMessage).toSnortLogData();
        snortLog.setHash(hash);
